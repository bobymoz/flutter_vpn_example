...
class VpnServer {
  final String name;
  final String countryCode; // e.g. "US", "JP"
  final String flagEmoji;
  final VpnProtocol protocol;
  final String endpoint;   // WireGuard only
  final String config;     // WireGuard: wg-quick, OpenVPN: .ovpn content
  final int ping;          // ms (0 = unknown, for WG servers)
  final int speedBps;      // bytes/s (0 = unknown, for WG servers)
...
// ─────────────────────────────────────────────
// YOUR WIREGUARD SERVER  (always shown first)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// VPN GATE FETCHER
// Fetches the live CSV from VPN Gate and returns
// only servers that are currently online (ping > 0).
// ─────────────────────────────────────────────
class VpnGateFetcher {
  ...
  static Future<List<VpnServer>> fetch() async {
    try {
      final response =
          await http.get(Uri.parse(_apiUrl)).timeout(const Duration(seconds: 12));
      ...
      for (final line in lines) {
        ...
        // 6=CountryShort,7=NumVpnSessions,8=Uptime,9=TotalUsers,
        // 10=TotalTraffic,11=LogType,12=Operator,13=Message,
        // 14=OpenVPN_ConfigData_Base64
        if (cols.length < 15) continue;

        final countryCode = cols[6].trim().toUpperCase();
        final pingStr = cols[3].trim();
        final speedStr = cols[4].trim();
        final ovpnBase64 = cols[14].trim();
        final countryLong = cols[5].trim();

        // Skip if no OpenVPN config
        if (ovpnBase64.isEmpty) continue;

        final int ping = int.tryParse(pingStr) ?? 0;
        final int speed = int.tryParse(speedStr) ?? 0;

        // IMPORTANT: skip servers with ping = 0 (offline / no data)
        if (ping == 0) continue;

        // Decode the .ovpn config
        String ovpnConfig;
        try {
      ...
      // Limit to top 30 servers to avoid overwhelming the UI
                ...
                Opacity(
                  ...
                  child: Column(children: [
                    ...
                    const Text('SECURE VPN',
                        style: TextStyle(
                            fontSize: 11, letterSpacing: 5, color: Colors.white54)),
                  ]),
                ),
...
class _MainScreenState extends State<MainScreen>
  ...
  void _onConnectTapped() {
    ...
    setState(() { _adFlowRunning = true; _loadingAd = true; });
  ...
  void _waitForAdThenShow() {
    ...
    Timer.periodic(const Duration(milliseconds: 300), (t) {
      ...
      if (_interstitialReady) { t.cancel(); _showInterstitial(); }
      else if (waited >= 8000) { t.cancel(); _afterAd(); }
    });
  }
  ...
  void _afterAd() {
    setState(() { _loadingAd = false; _adFlowRunning = false; });
    _connectSelected();
    _loadInterstitial();
  }
  ...
  bool get _isVpnBusy {
    if (_selectedServer.protocol == VpnProtocol.wireguard) {
      return _wgStage != VpnStage.connected && _wgStage != VpnStage.disconnected;
    }
    return _ovpnStage != VPNStage.connected &&
        _ovpnStage != VPNStage.disconnected &&
        _ovpnStage != VPNStage.noConnection;
  }
          ...
          child: Column(
            children: [
              ...
              UnityBannerAd(
                ...
                onClick: (_) {},
                onFailed: (_, __, msg) => debugPrint('[Ads] Banner failed: $msg'),
              ),
            ],
          ),
        ),
  ...
  Widget _buildLogo() {
    return Column(children: [
        ...
        child: CircleAvatar(
          ...
          child: ClipOval(
            child: Image.asset('assets/LOGO.png',
                ...
                errorBuilder: (_, __, ___) => const Icon(
                    Icons.shield, color: Color(0xFFFF8C5A), size: 56)),
          ),
        ),
      ),
      ...
      ShaderMask(
        shaderCallback: (b) => const LinearGradient(
            colors: [Color(0xFFFF8C5A), Color(0xFFFFD580)]).createShader(b),
        child: const Text('NOCIX',
            style: TextStyle(
              ...
              boxShadow: [
                BoxShadow(
                  color: (_isConnected
                      ...
                      : _isBusy ? Colors.orange : const Color(0xFFE8622A))
                      .withOpacity(0.5),
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(_sessionTime,
                style: const TextStyle(
                    ...
                    color: Color(0xFF00FF9D), letterSpacing: 2,
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    _sectionHeader('⚡  Premium  •  WireGuard UDP 53'),
                    _serverTile(kMyServer, ctx),

                    // ── VPN GATE SERVERS ──
                    _sectionHeader('🌐  VPN Gate  •  OpenVPN'),
                    ...
                    else if (_serversFetched && _vpnGateServers.isEmpty)
                      Padding(
                        ...
                        child: Column(children: [
                          ...
                          TextButton.icon(
                            ...
                            label: const Text('Retry',
                                style: TextStyle(color: Color(0xFFFF8C5A))),
                            onPressed: () {
                              Navigator.pop(ctx);
                              _fetchVpnGateServers();
                              Future.delayed(
                    ...
                    else if (!_serversFetched && !_loadingServers)
                      // First open before fetch completes
                      const SizedBox()
                    else
                      ..._vpnGateServers
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () { Navigator.pop(ctx); _openServerPicker(); },
            ),
            ListTile(
              leading: const Icon(Icons.refresh,
                  color: Color(0xFFFF8C5A)),
              title: const Text('Refresh VPN Gate Servers'),
              onTap: () { Navigator.pop(ctx); _fetchVpnGateServers(); },
            ),
            ListTile(
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(ctx);
                // Open your terms URL
                // launchUrl(Uri.parse('https://www.jinoca.com/demo.html'));
              },
            ),
            const SizedBox(height: 24),
          ],
    ...
    if (result != null) {
      ...
      setState(() {
        _selectedServer = VpnServer(
          ...
          config: content,
          endpoint: isWg
              ? RegExp(r'Endpoint\s*=\s*(.+)').firstMatch(content)?.group(1)?.trim() ?? ''
              : '',
        );
      });
    }
